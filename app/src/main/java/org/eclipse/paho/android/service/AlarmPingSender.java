...
class AlarmPingSender implements MqttPingSender {
    ...
    public void start() {
        if (hasStarted) {
            Log.d(TAG, "AlarmPingSender already started, skip re-register");
    ...
    public void schedule(long delayInMilliseconds) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            ...
            if (alarmManager.canScheduleExactAlarms()) {
                ...
                Log.d(TAG, "Alarm schedule using setAndAllowWhileIdle (no exact alarm permission), next: " + delayInMilliseconds);
                alarmManager.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, nextAlarmInMilliseconds, pendingIntent);
            }
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            // In SDK 23-30, setExactAndAllowWhileIdle works without special permission.
            Log.d(TAG, "Alarm schedule using setExactAndAllowWhileIdle, next: " + delayInMilliseconds);
            alarmManager.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, nextAlarmInMilliseconds, pendingIntent);
        } else {
    ...
    class AlarmReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            wakelock.acquire(comms.getKeepAlive());
